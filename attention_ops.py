...
from rnn_util import ZoneoutWrapper
...
class AttentionCell(tf.nn.rnn_cell.RNNCell):
    def __init__(self, feature_extractor, n_units, att_gain, glimpse_size,
                 input_size=None, batch_size=None, zoneout_prob=0.,
                 attention_module=RATMAttention, normalize_glimpse=False,
                 identity_init=True, is_training=True):
        ...
        self.attention_unit = attention_module(self.input_size, self.glimpse_size)
    ...
    @property
    def att_size(self):
        return self.attention_unit.n_params
    ...
    def zero_state(self, f, dtype, bbox0=None, presence=None, img0=None,
                   transform_featuers=False, transform_state=False):
        if bbox0 is None:
            return super(AttentionCell, self).zero_state(batch_size, dtype)

        self.att0 = self.attention_unit.bbox_to_attention(bbox0)
    ...
    def _zero_state(self, img, att, presence, state, transform_features, transform_state=False):
        ...
        with tf.variable_scope(self.__class__.__name__) as vs:
            ...
            if transform_features:
                ...
                features_flat = keras.layers.dense(features_flat, self.n_units, name='init_feature_transform')
            ...
            if transform_state:
                for i, hs in enumerate(hidden_state):
                    ...
                    hidden_state[i] = keras.layers.dense(hs, self.n_units, name=name)
    ...
    def _make_cell(self, is_training):
        ...
        raw_cell = tf.nn.rnn_cell.LSTMCell
    ...
    def extract_features(self, inpt, raw_att, apperance_vec=None, reuse=False):
        ...
        raw_att_flat, att_flat, glimpse_flat = self.attention_unit.extract_glimpse(inpt, raw_att,
                                                                              return_all=True)
